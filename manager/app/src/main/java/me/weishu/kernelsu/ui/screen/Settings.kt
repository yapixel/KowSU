            ...
            KsuIsValid() {
                ...
                LaunchedEffect(Unit) {
                    withContext(Dispatchers.IO) {
                        noMountEnabled = Shell.cmd("test -f $noMountFlag").exec().isSuccess
                    }
                }
                SwitchItem(
                    ...
                    scope.launch(Dispatchers.IO) {
                        val command = if (checked) "touch" else "rm -f"
                        val result = Shell.cmd("$command $noMountFlag").exec()
                        if (result.isSuccess) {
                ...
                LaunchedEffect(Unit) {
                    withContext(Dispatchers.IO) {
                        noTmpfsEnabled = Shell.cmd("test -f $noTmpfsFlag").exec().isSuccess
                    }
                }
                SwitchItem(
                    ...
                    scope.launch(Dispatchers.IO) {
                        val command = if (checked) "touch" else "rm -f"
                        val result = Shell.cmd("$command $noTmpfsFlag").exec()
                        if (result.isSuccess) {
